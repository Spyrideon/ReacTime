...
GraphScreen::GraphScreen() : graphBackground(graphTexture){
	if (!graphTexture.loadFromFile("../assets/textures/GraphScreen.png"))			// initialize textures
	...
	vertexVector.reserve(100);			// initialize vector that stores vertices
	//writeVertex({ 0.f,0.f });
}
...
void GraphScreen::writeVertex(sf::Vector2f pos) {
	if (vertexVector.size() >= 100)			// protect from overflow
		return;

	sf::Sprite sprite(vertexTexture);		// initialize new vertex sprite
	...
	vertexVector.emplace_back(sprite);		// then place in vector
}